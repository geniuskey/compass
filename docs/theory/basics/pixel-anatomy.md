...
The BARL is a multi-layer thin-film stack of dielectrics with refractive indices intermediate between the color filter and silicon. Common building blocks include silicon dioxide (SiO2, n ~ 1.46), silicon nitride (Si3N4, n ~ 2.0), hafnium oxide (HfO2, n ~ 2.0), aluminum oxide (Al2O3, n ~ 1.7), titanium dioxide (TiO2, n ~ 2.4), and tantalum pentoxide (Ta2O5, n ~ 2.1) — and individual vendors mix and stack these very differently. The exact material set, the number of layers (typically 2 to 5), and the stacking order are part of each vendor's process recipe and are rarely disclosed; they are tuned together with the photodiode design and the target spectral band. Each layer is on the order of a few tens of nanometers thick, and the thicknesses are co-optimized so that reflections at the multiple interfaces partially cancel.

In simple terms: the BARL replaces a single abrupt index jump (filter → silicon) with a series of smaller, carefully phased jumps so that the reflected waves from the various interfaces interfere destructively. There is no single "correct" stack — many different material orderings can achieve similar broadband anti-reflection performance, which is why every vendor's choice is its own engineering trade-off rather than a textbook answer.

A single-layer ARC can be optimized for one specific wavelength. But pixels need to work across a broad wavelength range (the full band of the color filter above). Multi-layer designs typically use 2 to 5 alternating films to cover the visible spectrum, at the cost of more complex manufacturing. The default BARL stack shipped in the COMPASS sample configs is just one *illustrative* choice; you should treat it as a starting point and re-optimize the stack for whatever silicon thickness, color filter, and target QE band you are actually modeling.
