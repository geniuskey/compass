...
Most numerical methods for Maxwell's equations are formulated in terms of permittivity rather than refractive index. COMPASS stores materials in $(n, k)$ form in `MaterialDB`; how the conversion to $\varepsilon$ is used inside each solver is covered in [Optical Simulation](../simulation/).
