...
이 섹션은 개념과 mental model을 다룹니다. 실행 절차는 [가이드](/ko/guide/), 생성된 검증 근거는 [리포트](/ko/reports/), 정확한 클래스와 설정 필드는 [API 레퍼런스](/ko/reference/)에서 다룹니다.

## 챕터 구성

| 챕터 | 범위 | 이런 분께 추천 |
|---|---|---|
| [입문](./basics/what-is-cmos-sensor.md) | CIS 픽셀, 광학, QE를 수식 없이 잡는 개관 | 깊은 이론 페이지로 가기 전에 직관을 만들고 싶을 때 |
...
- 설치와 command-line workflow는 [가이드](/ko/guide/)에서 다룹니다.
- 생성된 benchmark evidence는 [리포트](/ko/reports/)에서 다룹니다.
- API signature와 YAML schema 세부사항은 [API 레퍼런스](/ko/reference/)에서 다룹니다.

## 추천 학습 경로

**이미지 센서 광학을 처음 접하는 경우**
1. [CMOS 이미지 센서란?](./basics/what-is-cmos-sensor.md) -> [초보자를 위한 광학 입문](./basics/optics-primer.md) -> [픽셀 구조 분석](./basics/pixel-anatomy.md)
