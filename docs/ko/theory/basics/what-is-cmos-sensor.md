---
title: CMOS 이미지 센서란?
description: CMOS 이미지 센서, 픽셀, BSI/FSI 구조, Bayer 색상, QE, 크로스토크, 광학 시뮬레이션의 필요성을 쉬운 언어로 설명합니다.
---
...
::: info 이 페이지의 범위
이 페이지는 가장 높은 수준의 개관입니다. CMOS 이미지 센서가 무엇이며 COMPASS가 왜 픽셀을 시뮬레이션하는지 설명합니다. 실제 픽셀 스택은 [픽셀 해부학](./pixel-anatomy.md) 또는 [이미지 센서 광학](../sensor/image-sensor-optics.md)에서 이어서 다루고, 솔버 계산 방식은 [광학 시뮬레이션](../simulation/)에서 다룹니다.
:::
...
- **[픽셀 해부학](./pixel-anatomy.md)** -- 현대 BSI 픽셀의 각 층과 그 역할을 차례로 살펴봅니다.
- **[첫 번째 시뮬레이션](/ko/guide/first-simulation)** -- 단계별 가이드와 함께 첫 COMPASS 시뮬레이션을 바로 실행해 봅니다.
- **[설치 가이드](/ko/guide/installation)** -- COMPASS와 솔버 백엔드를 설치합니다.
