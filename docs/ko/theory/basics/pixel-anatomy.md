---
title: 픽셀 해부학
description: BSI CMOS 이미지 센서 픽셀 스택을 마이크로렌즈, 컬러 필터, BARL, 실리콘, 포토다이오드, DTI, 주요 광 손실 중심으로 설명합니다.
---
...
::: info 이 페이지의 범위
이 페이지는 픽셀 스택을 시각적으로 따라가는 입문 안내입니다. 같은 구성 요소의 압축된 기술 모델은 [이미지 센서 광학](../sensor/image-sensor-optics.md), 흡수된 빛을 성능 지표로 바꾸는 공식은 [양자 효율](../sensor/quantum-efficiency.md)에서 다룹니다.
:::
...
BARL은 컬러 필터와 실리콘의 중간 굴절률을 갖는 유전체들로 이루어진 다층 박막 스택입니다. 자주 쓰이는 재료로는 이산화규소(SiO2, n ~ 1.46), 질화규소(Si3N4, n ~ 2.0), 산화하프늄(HfO2, n ~ 2.0), 산화알루미늄(Al2O3, n ~ 1.7), 이산화티타늄(TiO2, n ~ 2.4), 오산화탄탈(Ta2O5, n ~ 2.1) 등이 있는데, 어떤 재료를 어떤 순서로 몇 층 쌓는지는 **벤더마다 크게 다릅니다**. 사용 재료 조합, 층 수(보통 2~5층), 적층 순서는 공정 레시피의 일부로 거의 공개되지 않으며, 포토다이오드 설계 및 타깃 분광 대역과 함께 동시에 튜닝됩니다. 각 층의 두께는 대략 수십 나노미터 수준이며, 여러 계면에서 반사된 파동이 서로 상쇄되도록 두께들이 함께 최적화됩니다.

간단히 말해: BARL은 필터-실리콘 사이의 한 번의 급격한 굴절률 도약을, 여러 개의 작고 위상이 잘 맞춰진 도약으로 분산시켜 각 계면의 반사파가 상쇄 간섭을 일으키도록 만든 구조입니다. 단 하나의 "정답" 스택이 존재하는 것이 아니라, 서로 다른 재료 조합과 적층 순서로도 비슷한 광대역 반사 방지 성능을 얻을 수 있기 때문에, 벤더별 선택은 교과서적 정답이 아니라 각자의 엔지니어링 트레이드오프의 결과입니다.

단일층 ARC는 하나의 특정 파장에 대해 최적화할 수 있습니다. 하지만 픽셀은 넓은 파장 범위(위의 컬러 필터의 전체 대역)에서 작동해야 합니다. 가시광 스펙트럼 전체를 커버하기 위해 보통 2~5층의 교대 막을 사용하며, 층 수가 많아질수록 제조 복잡도가 증가합니다. COMPASS 의 샘플 설정에 들어 있는 기본 BARL 스택은 어디까지나 *예시* 일 뿐이며, 실제로 모델링하려는 실리콘 두께, 컬러 필터, 타깃 QE 대역에 맞게 다시 최적화해야 합니다.
