---
title: 양자 효율
description: COMPASS의 양자 효율 공식 기준 문서로 흡수 전력 적분, 포인팅 플럭스, 픽셀별 QE, 크로스토크, 에너지 보존을 설명합니다.
---
...
[픽셀 광학 효과](./pixel-optical-effects.md)에서 어떤 설계 변수가 QE 곡선을 제한하는지 판단한 뒤, 좁은 범위의 스윕으로 확인합니다:

| QE 또는 크로스토크 증상 | 가능성이 큰 설계 변수 | 실무 워크플로 |
|---|---|---|
| 청색 QE가 낮고 가장자리 누설이 큼 | 마이크로렌즈 형상, 금속 격자 개구, 회절 | [마이크로렌즈 & CRA](/ko/cookbook/microlens-optimization) |
| 스펙트럼 리플 또는 넓은 반사 손실 | BARL 두께/재료 스택 | [BARL 설계](/ko/cookbook/barl-design) |
| 적색/NIR QE가 너무 빨리 감소 | 실리콘 두께, 포토다이오드 깊이, DTI 깊이 | [Si 흡수 깊이](/ko/simulator/si-absorption) |
| 색 채널이 이웃 픽셀로 번짐 | 금속 격자, CFA 흡수, DTI 형상 | [DTI 크로스토크](/ko/cookbook/dti-crosstalk) |
| 센서 모서리 픽셀이 저하됨 | CRA, 마이크로렌즈 오프셋, 각도 응답 | [각도 응답](/ko/simulator/angular-response) |
