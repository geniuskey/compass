...
BARL은 컬러 필터와 실리콘 포토다이오드 사이에 위치합니다. 반사 방지 코팅이 없으면 공기/SiO2/실리콘 계면에서 입사광의 15--30%가 반사되어 QE를 크게 감소시킵니다. 잘 설계된 BARL 스택은 가시광선 전 영역에서 이 반사를 5% 이하로 줄여줍니다. 실제 BARL 레시피(사용 재료, 층 수, 적층 순서)는 벤더마다 크게 다르고 대부분 비공개이므로, 아래에서 사용하는 스택은 설계 스윕의 *출발점* 일 뿐 표준 레시피가 아닙니다.
