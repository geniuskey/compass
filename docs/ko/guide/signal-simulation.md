...
픽셀 설계의 양자 효율(QE) 스펙트럼을 계산한 후, 다음 단계는 실제 이미징 조건에서의 신호 수준을 예측하는 것입니다. 이 가이드는 실용적·코드 중심의 워크스루입니다.

::: tip 이론 배경
QE를 전자로 변환하는 적분과 그 기저 모델 — 광원 SPD, 장면 반사율, 렌즈 투과율 — 은 [신호 체인](/ko/theory/sensor/signal-chain)을 참고하세요. 노이즈와 SNR 측면은 [노이즈, SNR, 다이나믹 레인지](/ko/theory/sensor/noise-and-snr)를 참고하세요.
:::
